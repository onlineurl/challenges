...
const PhotoModal: React.FC<PhotoModalProps> = ({ photo, onClose, onReject }) => {
    const handleReject = () => {
        if (window.confirm(`¿Seguro que quieres rechazar la foto de ${photo.participant_name}? Se borrará la foto y se restarán los puntos.`)) {
            onReject(photo.id);
        }
    }
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl relative" onClick={(e) => e.stopPropagation()}>
                <button onClick={onClose} className="absolute -top-3 -right-3 p-2 bg-white rounded-full text-slate-600 hover:text-slate-900 shadow-lg" aria-label="Cerrar visor"><X className="w-6 h-6" /></button>
                <img src={photo.media_url} alt={`Foto de ${photo.participant_name}`} className="w-full h-auto object-contain rounded-t-lg max-h-[70vh]" />
                <div className="p-4 bg-slate-50 rounded-b-lg flex justify-between items-center">
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1 text-slate-600"><Target className="w-4 h-4"/> <p>Reto: <span className="font-medium">{photo.challenge_title}</span></p></div>
                    </div>
    ...
    <>
      ...
      <div className="p-4 md:p-6 bg-slate-50 min-h-screen">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Galería de Fotos</h1>
            <p className="text-slate-500 mt-1">Fotos de "{event.title}"</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {photos.map(photo => (
              <div key={photo.id} className="group relative cursor-pointer overflow-hidden rounded-lg shadow-md" onClick={() => setSelectedPhoto(photo)}>
                <img src={photo.media_url} alt={`Foto de ${photo.participant_name}`} className="w-full h-full object-cover aspect-square transition-transform duration-300 group-hover:scale-110" />
           ...
           <div className="text-center py-12 border-2 border-dashed border-slate-300 rounded-lg bg-white">
              <h3 className="text-xl font-semibold text-slate-700">¡Aún no hay fotos!</h3>
              <p className="text-slate-500 mt-2">Las fotos aparecerán aquí cuando los invitados completen retos.</p>
            </div>
        )}
      </div>
    </>
